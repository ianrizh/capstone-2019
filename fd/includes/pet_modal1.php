<div class="modal fade" id="new_pet">
...
<label for="email" class="col-sm-3 control-label">PET NAME</label>

<div class="col-sm-9">
<input class='form-control' type='text' name="pet_name" required autofocus autocomplete="off">
</div>
</div>
<div class="form-group">
...
<br>
<form class="form-horizontal">
<div class='box-header with-border' style='margin-left:-15px;'>
...
$date=date('Y-m-d');
?>
<input type='hidden' name='thedate' value='<?php echo $date ?>'>
<input class='form-control' type='text' value='<?php echo date('M. d, Y', strtotime($date)) ?>' readonly>
</div>
</div>
...
<div class='col-sm-9'>
<select id='type' class='form-control' onChange='java_script_:show2(this.options[this.selectedIndex].value)' required>
<option value='' disabled selected required>---Select---</option>
<option value='VHC1'>Veterinary Health Care</option>
<option value='Grooming1'>Grooming Services</option>
</select>
</div>
</div>
<?php
...
<label for="edit_name" class="col-sm-3 control-label" id="v1" style="display:none">TIME</label>
<div class="col-sm-9" style="margin-bottom:-50px;">
<select class="form-control" id="h1" style="display:none" name="time_reservation" required>
<option value="" disabled selected required>---Select---</option>';
...
echo "
<div class='form-group'>
<label for='email' class='col-sm-3 control-label' id='groominga' style='display: none; margin-top:-16px;'>GROOMING TYPE</label>

<div class='col-sm-9' style='margin-top:-16px'>
<select class='form-control' id='groomingb' onChange='onSelect3(this.value)' required style='display: none;'>
<option value='' disabled selected required>---Select---</option>"; 
...
$stmt = $conn->prepare("SELECT * FROM services where id_category='$id_category'");
$stmt->execute();

foreach($stmt as $crow){ 
	$id_services = $crow['id_services'];
echo "
<option value='".$id_services."'>".$crow['name']."</option>
";
}
}
echo"
...
<script>
 function onSelect3(str){
  ...
  srvc.onreadystatechange=function(){
    document.getElementById("details3").innerHTML=this.responseText;
  }
  srvc.open('GET', "g_time.php?id_services="+str,true);
  srvc.send();
 }
</script>
</div>
