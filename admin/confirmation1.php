...
{
try{
$stmt = $conn->prepare("UPDATE online_reservation1 SET status='$st' where reservation_id = $reservation_id");
...
{
try{
$stmt = $conn->prepare("UPDATE online_reservation1 SET status='$st1' where reservation_id = $reservation_id");
$stmt->execute(['status'=>$st]);
$message = "
<h2>WE'RE SORRY!</h2>
